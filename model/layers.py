...
class SpatialConvLayer(nn.Module):
    ...
    def __init__(self, c_in, c_out, gso, Ks, bias, G, graph_conv_type):
        ...
        if self.graph_conv_type == 'cheb_graph_conv':
            self.layers = dglnn.ChebConv(c_out, c_out, Ks, F.relu, bias)
        elif self.graph_conv_type == 'graph_conv':
            self.layers = dglnn.GraphConv(c_out, c_out, activation=F.relu, bias=bias)
    ...
    def forward(self, x):
        ...
        x_gc_in = x_gc_in.transpose(0, 3)
        # [nodes, c_in, ts, batch] --> [nodes, batch, ts, c_in]
        x_gc_in = x_gc_in.transpose(1, 3)
        # output: [nodes, batch, ts, c_out]

        if self.graph_conv_type == 'cheb_graph_conv':

            output = self.layers(self.G, x_gc_in)

        elif self.graph_conv_type == 'graph_conv':
            output = self.layers(self.G, x_gc_in)

        # output = self.layers(self.G, x)
        # output
        # 源程序中有残差
        output = torch.add(output, x_gc_in)
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate, G):
        super(STConvBlock, self).__init__()
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], n_vertex, act_func)
        self.graph_conv = SpatialConvLayer(channels[0], channels[1], gso, Ks, bias, G, graph_conv_type)

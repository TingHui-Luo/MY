...
def get_parameters():
    ...
    parser.add_argument('--dataset', type=str, default='pemsd7-m-10', choices=['metr-la', 'pems-bay', 'pemsd7-m'])
    ...
    parser.add_argument('--epochs', type=int, default=5, help='epochs, default as 50')
...
def data_preparate(args, device):
    adj, n_vertex, adj_matrix = dataloader.load_adj(args.dataset)
    ...
    G = dgl.from_scipy(sp_mx).to(device)

    return n_vertex, zscore, train_iter, val_iter, test_iter, G


def prepare_model(args, blocks, n_vertex, G):
    loss = nn.MSELoss()
    es = earlystopping.EarlyStopping(mode='min', min_delta=0.0, patience=args.patience)

    if args.graph_conv_type == 'cheb_graph_conv':
        model = models.STGCNChebGraphConv(args, blocks, n_vertex, G).to(device)
    else:
        model = models.STGCNGraphConv(args, blocks, n_vertex, G).to(device)
...
@torch.no_grad()
def ts(zscore, loss, model, test_iter, args):
    model.eval()
...
if __name__ == "__main__":
    ...
    n_vertex, zscore, train_iter, val_iter, test_iter, G = data_preparate(args, device)
    ...
    loss, es, model, optimizer, scheduler = prepare_model(args, blocks, n_vertex, G)
    train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter)
    ts(zscore, loss, model, test_iter, args)
